...
- 图片复制到 `./${filename}.assets` 文件夹

当该设置开启后，Typora 会在侧边栏里出现大量 `*.assets` 目录，本插件就是为这个场景设计的。

## 使用前后对比

### 使用前（未启用插件）

![Before](./docs/assets/before.svg)

### 使用后（启用插件）

![After](./docs/assets/after.svg)

> 说明：插件只隐藏侧边栏显示，不会删除磁盘中的 `.assets` 文件夹和图片文件。
